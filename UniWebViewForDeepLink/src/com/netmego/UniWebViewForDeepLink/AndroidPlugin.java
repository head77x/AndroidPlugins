...
public class AndroidPlugin extends UnityPlayerActivity
{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (UniWebView.defaultUserAgent == null) {
            WebView w = new WebView(getUnityActivity_());
            UniWebView.defaultUserAgent = w.getSettings().getUserAgentString();;
        }
    }
    ...
    public static void _UniWebViewSetCookie(final String url, final String cookie) {
        setCookie(url, cookie);
    }

    public static String _UniWebViewGetCookie(final String url, final String key) {
    ...
    public static String _UniWebViewGetUserAgent(final String name) {
        ...
        if (UniWebView.customUserAgent != null) {
            return UniWebView.customUserAgent;
        }
    ...
    public static void _UniWebViewAddCertTrustedHost(final String name, final String host) {
        runSafelyOnUiThread(new Runnable() {
            @Override
            public void run() {
                UniWebViewDialog dialog = UniWebViewManager.Instance().getUniWebViewDialog(name);
                if (dialog != null) {
                    dialog.addCertTrustedHose(host);
                }
            }
        });
    }

    public static void _UniWebViewAllowThirdPartyCookies(final boolean allowed) {
        runSafelyOnUiThread(new Runnable() {
            @Override
            public void run() {
                UniWebView.acceptThirdPartyCookie = allowed;

                if (Build.VERSION.SDK_INT >= 21) {
                    CookieManager manager = CookieManager.getInstance();
                    for (UniWebViewDialog dialog: UniWebViewManager.Instance().allDialogs()) {
                        manager.setAcceptThirdPartyCookies(dialog.getWebView(), allowed);
                    }
                }
            }
        });
